...
import { IoSearchOutline, IoChevronDown, IoCloseOutline, IoLogoFacebook, IoLogoTwitter, IoLogoYoutube } from "react-icons/io5";
...
const Header = () => {
    ...
    const navItems = [
        { name: 'Home', children: [{ name: 'Home Layout 1', href: '/' }, { name: 'Home Layout 2', href: '/' }] },
        { name: 'Politics', children: [{ name: 'National', href: '/politics/national' }, { name: 'International', href: '/politics/international' }] },
        { name: 'News', href: '/news' },
    ...
    return (
        <header className="relative bg-white z-60">
            <div className="flex items-center justify-between h-[70px] md:h-[90px] border-b border-black px-4 relative bg-white z-10">
                ...
                <nav className="hidden md:flex space-x-6  font-bold uppercase font-condensed tracking-wider py-3 w-full justify-center">
                    {navItems?.map((item) => (
                        <div key={item.name} className="relative group">
                            ...
                            >
                                ...
                                {Array.isArray(item.children) && (
                                    <IoChevronDown className="size-3 text-gray-400" />
                                )}
                            </Link>
            ...
            <div className={`absolute left-0 right-0 md:left-auto md:right-4 md:top-full md:w-[400px] bg-white border-b md:border border-gray-100 md:shadow-2xl transition-all duration-300 overflow-visible z-100 ${isSearchOpen ? 'max-h-[150px] opacity-100 py-6 px-4 md:py-4 md:px-4' : 'max-h-0 opacity-0 py-0 px-4 invisible'}`}>
            ...
            <div className={`fixed inset-0 z-100 ${isMenuOpen ? 'visible' : 'invisible'} transition-[visibility] duration-300`}>
                <div 
                ...
                <div className={`absolute inset-y-0 left-0 w-[300px] bg-white shadow-2xl flex flex-col p-8 overflow-y-auto transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    ...
                    <div className="mt-auto pt-10">
                        <div className="flex space-x-5 mb-8">
                            <Link href="#" className="text-gray-400 hover:text-[#3b5998] transition-colors"><IoLogoFacebook className="size-5" /></Link>
                            <Link href="#" className="text-gray-400 hover:text-[#1da1f2] transition-colors"><IoLogoTwitter className="size-5" /></Link>
                            <Link href="#" className="text-gray-400 hover:text-[#ff0000] transition-colors"><IoLogoYoutube className="size-5" /></Link>
                            <Link href="#" className="text-gray-400 hover:text-[#ee802f] transition-colors"><FaRss className="size-5" /></Link>
                        </div>
                        
                        <div className="space-y-1">
                            <p className="text-[10px] text-gray-400 leading-relaxed font-medium">
                                © 2026 JNews - Premium WordPress news & magazine theme by <Link href="#" className="hover:text-black">Jegtheme</Link>.
                            </p>
                        </div>
                    </div>
                </div>
